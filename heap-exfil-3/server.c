...
char *ROOT_STORE_DATA_FILE;
char *USER_STORE_DATA_FILE;
...
static void usage(char ** argv) {
	printf("./%s [-a <ALPHA/FILE/PATH>] [-b <BETA/FILE/PATH>] [-e <EUID>] [-d] [-o] [-h] [-r]\n", argv[0]);
	printf("  -a <FILE/PATH>: struct access alpha (private) file path.\n");
	printf("  -b <FILE/PATH>: struct access beta (public) file path.\n");
	printf("  -p <PORT>: port number for the local server to listen on.");
	printf("  -e <EUID>: a user id (as required int) to change to after the libc:bind on port.\n");
	...
	printf("  -s: enable protocol op_storedata\n");
...
int main (int argc, char ** argv) {
	...
	bool enable_op_storedata = false;
	int opt;
	...
	while ((opt = getopt(argc, argv, "dohrsp:e:a:b:")) != -1) {
        	switch (opt) {
						/* Set the struct access data files */
						case 'a':
	 						ROOT_STORE_DATA_FILE = optarg;
							printf("Set admin storedata file to: %s\n", ROOT_STORE_DATA_FILE);
							break;
						case 'b':
	 						USER_STORE_DATA_FILE = optarg;
							printf("Set user storedata file to: %s\n", USER_STORE_DATA_FILE);
							break;
            ...
            case 's': enable_op_storedata = true; break;
        		default: break;
	        }
	}
